(function() {
    ...
    // 3. Inject HTML (WhatsApp Meta UI style)
...
(function() {
    ...
    // 2. Inject CSS (Black and White Theme)
    var style = document.createElement('style');
    style.innerHTML = '#ai-chat-widget-btn { position: fixed; bottom: 20px; right: 20px; z-index: 9999; background: #000; color: #FFF; border: 1px solid #333; border-radius: 40px; cursor: pointer; box-shadow: 0 4px 15px rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; padding: 12px; transition: all 0.5s ease; overflow: hidden; } #ai-chat-widget-btn:hover { transform: scale(1.05); border-color: #FFF; } #ai-btn-text { font-family: sans-serif; font-size: 15px; font-weight: 600; white-space: nowrap; overflow: hidden; transition: all 0.5s ease; max-width: 100px; opacity: 1; margin-left: 10px; margin-right: 5px; color: #FFF; } .shrink-text { max-width: 0 !important; opacity: 0 !important; margin: 0 !important; } #ai-chat-window { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #FFF; z-index: 10000; flex-direction: column; font-family: sans-serif; } #ai-chat-header { background: #000; color: #FFF; padding: 15px 20px 25px 20px; display: flex; align-items: center; border-bottom-left-radius: 40px; border-bottom-right-radius: 40px; box-shadow: 0 4px 10px rgba(0,0,0,0.2); } #ai-chat-close { background: none; border: none; color: #FFF; font-size: 24px; cursor: pointer; padding-right: 15px; font-weight: bold; } #ai-chat-logo { width: 40px; height: 40px; border-radius: 50%; background: #FFF; margin-right: 15px; object-fit: cover; border: 1px solid #333; } #ai-chat-title { font-size: 18px; font-weight: bold; letter-spacing: 1px; text-transform: uppercase; } #ai-chat-messages { flex-grow: 1; padding: 20px; overflow-y: auto; background: #FFF; display: flex; flex-direction: column; gap: 15px; } .msg-wrapper { display: flex; flex-direction: column; max-width: 85%; } .msg-label { font-size: 11px; color: #888; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 0.5px; } .user-wrapper { align-self: flex-end; align-items: flex-end; } .user-msg { background: #000; color: #FFF; padding: 12px 18px; border-radius: 20px 20px 0 20px; font-size: 15px; line-height: 1.4; border: 1px solid #333; } .ai-wrapper { align-self: flex-start; align-items: flex-start; } .ai-msg { background: #F0F0F0; color: #000; padding: 12px 18px; border-radius: 20px 20px 20px 0; font-size: 15px; line-height: 1.4; min-height: 20px; border: 1px solid #E0E0E0; } .ai-msg p { margin: 0 0 10px 0; } .ai-msg p:last-child { margin: 0; } .ai-msg ul { margin: 0; padding-left: 20px; } .ai-msg strong { font-weight: bold; color: #000; } #ai-chat-input-area { display: flex; padding: 15px; background: #FFF; border-top: 1px solid #EEE; padding-bottom: 20px; align-items: center; gap: 10px; } #ai-chat-input { flex-grow: 1; background: #F0F0F0; border: 1px solid #E0E0E0; padding: 15px 20px; border-radius: 30px; outline: none; font-size: 15px; color: #000; } #ai-chat-send { background: #000; color: #FFF; border: none; width: 45px; height: 45px; border-radius: 50%; cursor: pointer; display: flex; justify-content: center; align-items: center; transition: background 0.3s; } #ai-chat-send:hover { background: #333; } #ai-chat-send svg { width: 20px; height: 20px; fill: #FFF; } .typing-indicator { display: flex; gap: 5px; padding: 5px 10px; } .dot { width: 8px; height: 8px; background: #888; border-radius: 50%; animation: bounce 1.4s infinite ease-in-out both; } .dot:nth-child(1) { animation-delay: -0.32s; } .dot:nth-child(2) { animation-delay: -0.16s; } @keyframes bounce { 0%, 80%, 100% { transform: scale(0); } 40% { transform: scale(1); } }';
    document.head.appendChild(style);

    // 3. Inject HTML (ESGaming Branding)
    var widgetContainer = document.createElement('div');
(function() {
    // 1. Dynamically Load Markdown Parser
    ...
    var style = document.createElement('style');
    style.innerHTML = `
        #ai-chat-widget-btn { position: fixed; bottom: 20px; right: 20px; z-index: 9999; background: #000; color: #FFF; border: 1px solid #333; border-radius: 40px; cursor: pointer; box-shadow: 0 4px 15px rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; padding: 12px; transition: all 0.5s ease; overflow: hidden; }
        #ai-chat-widget-btn:hover { transform: scale(1.05); border-color: #FFF; }
        #ai-btn-text { font-family: sans-serif; font-size: 15px; font-weight: 600; white-space: nowrap; overflow: hidden; transition: all 0.5s ease; max-width: 100px; opacity: 1; margin-left: 10px; margin-right: 5px; color: #FFF; }
        .shrink-text { max-width: 0 !important; opacity: 0 !important; margin: 0 !important; }
        #ai-chat-window { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #FFF; z-index: 10000; flex-direction: column; font-family: sans-serif; }
        #ai-chat-header { background: #000; color: #FFF; padding: 15px 20px 25px 20px; display: flex; align-items: center; border-bottom-left-radius: 40px; border-bottom-right-radius: 40px; box-shadow: 0 4px 10px rgba(0,0,0,0.2); }
        #ai-chat-close { background: none; border: none; color: #FFF; font-size: 24px; cursor: pointer; padding-right: 15px; font-weight: bold; }
        #ai-chat-logo { width: 40px; height: 40px; border-radius: 50%; background: #FFF; margin-right: 15px; object-fit: cover; border: 1px solid #333; }
        #ai-chat-title { font-size: 18px; font-weight: bold; letter-spacing: 1px; text-transform: uppercase; }
        #ai-chat-messages { flex-grow: 1; padding: 20px; overflow-y: auto; background: #FFF; display: flex; flex-direction: column; gap: 15px; }
        .msg-wrapper { display: flex; flex-direction: column; max-width: 85%; }
        .msg-label { font-size: 11px; color: #888; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 0.5px; }
        .user-wrapper { align-self: flex-end; align-items: flex-end; }
        .user-msg { background: #000; color: #FFF; padding: 12px 18px; border-radius: 20px 20px 0 20px; font-size: 15px; line-height: 1.4; border: 1px solid #333; }
        .ai-wrapper { align-self: flex-start; align-items: flex-start; }
        .ai-msg { background: #F0F0F0; color: #000; padding: 12px 18px; border-radius: 20px 20px 20px 0; font-size: 15px; line-height: 1.4; min-height: 20px; border: 1px solid #E0E0E0; }
        .ai-msg p { margin: 0 0 10px 0; }
        .ai-msg p:last-child { margin: 0; }
        .ai-msg ul { margin: 0; padding-left: 20px; }
        .ai-msg strong { font-weight: bold; color: #000; }
        #ai-chat-input-area { display: flex; padding: 15px; background: #FFF; border-top: 1px solid #EEE; padding-bottom: 20px; align-items: center; gap: 10px; }
        #ai-chat-input { flex-grow: 1; background: #F0F0F0; border: 1px solid #E0E0E0; padding: 15px 20px; border-radius: 30px; outline: none; font-size: 15px; color: #000; }
        #ai-chat-send { background: #000; color: #FFF; border: none; width: 45px; height: 45px; border-radius: 50%; cursor: pointer; display: flex; justify-content: center; align-items: center; transition: background 0.3s; }
    ...
    container.innerHTML = `
        <button id="ai-chat-widget-btn">
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
                ...
                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z" fill="url(#monoGrad)"/>
                <circle cx="12" cy="12" r="3.5" fill="url(#monoGrad)"/>
            </svg>
            <div id="ai-btn-text">Ask AI</div>
        </button>
        <div id="ai-chat-window">
            <div id="ai-chat-header">
                <button id="ai-chat-close">&#10094;</button>
                <img id="ai-chat-logo" src="https://ui-avatars.com/api/?name=ES&background=fff&color=000&rounded=true" alt="Logo">
                <div id="ai-chat-title">ESGaming</div>
            </div>
            <div id="ai-chat-messages">
                <div class="msg-wrapper ai-wrapper">
                    <div class="msg-label">ESGaming AI</div>
                    <div class="ai-msg">Hello! How can I help you build your ultimate setup today?</div>
                </div>
            </div>
